...
MeshRenderer::MeshRenderer(const Mesh& m, const Material& mat) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
    glBufferData(GL_ARRAY_BUFFER, 3 * mesh.NumVerts() * sizeof(float), &mesh.Pos()[0], GL_STREAM_DRAW);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo[2]);
    glBufferData(GL_ARRAY_BUFFER, 2 * mesh.NumVerts() * sizeof(float), &mesh.UVs()[0], GL_STATIC_DRAW);
